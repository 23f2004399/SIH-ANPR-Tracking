...
class MultiCameraANPRPipeline:
    ...
    def _log_track_to_csv(self, camera_id: str, track_id: int, track_data: dict, fps: float):
        ...
        entry_ts = time.strftime("%H:%M:%S", time.gmtime(entry_sec)) + f".{int((entry_sec % 1) * 100):02d}"
        exit_ts = time.strftime("%H:%M:%S", time.gmtime(exit_sec)) + f".{int((exit_sec % 1) * 100):02d}"
        ...
        if is_confident:
            ...
            logger.info(
                ...
                f"Time: {entry_ts} -> {exit_ts} | 💾 Saved Verified Crops"
            )
        else:
            logger.info(
                f"[{camera_id}] 🏁 Track #{track_id} EXITED | "
                f"Plate: '{best_plate}' (Avg Conf: {avg_conf:.1f}%) | "
                f"Time: {entry_ts} -> {exit_ts} | ⏭️ Skipped crop saving (< {self.args.min_save_conf}% conf)"
            )
    ...
    def _render_frame(
        ...
        for track_id, t_data in active_tracks.items():
            if t_data["last_seen_frame"] == frame_idx:
                ...
                cv2.rectangle(vis_frame, (vx1, vy1), (vx2, vy2), box_color, 2)

                (tw, th), baseline = cv2.getTextSize(tag_text, cv2.FONT_HERSHEY_SIMPLEX, 0.52, 2)
                lbl_y1 = max(0, vy1 - th - 8)
                lbl_y2 = vy1
                lbl_x2 = min(w, vx1 + tw + 10)
        ...
        for px1, py1, px2, py2 in current_frame_plates:
            cv2.rectangle(vis_frame, (px1, py1), (px2, py2), (0, 255, 255), 2)

        # Modern Top-Left HUD
        hud_w, hud_h = 360, 110
        overlay = vis_frame.copy()
        cv2.rectangle(overlay, (15, 15), (15 + hud_w, 15 + hud_h), (18, 22, 28), -1)
        cv2.addWeighted(overlay, 0.78, vis_frame, 0.22, 0, vis_frame)
        cv2.rectangle(vis_frame, (15, 15), (15 + hud_w, 15 + hud_h), (50, 60, 75), 1)

        pct = (frame_idx / max(total_frames, 1)) * 100.0
        cv2.putText(vis_frame, f"NODE: {camera_id} | ZYRODEV ANPR", (26, 38), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (0, 200, 255), 2)
        cv2.putText(vis_frame, f"FRAME: {frame_idx}/{total_frames} ({pct:.1f}%)", (26, 62), cv2.FONT_HERSHEY_SIMPLEX, 0.44, (220, 220, 220), 1)
        cv2.putText(vis_frame, f"ACTIVE VEHICLES: {len(active_tracks)}", (26, 84), cv2.FONT_HERSHEY_SIMPLEX, 0.44, (100, 255, 100), 1)
        cv2.putText(vis_frame, f"VEHICLES LOGGED: {logged_count}", (26, 106), cv2.FONT_HERSHEY_SIMPLEX, 0.44, (255, 180, 50), 1)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Run OCR every N frames for active vehicles (default: 2)"
    )
    parser.add_argument(
        "--max_ocr_samples",
        type=int,
